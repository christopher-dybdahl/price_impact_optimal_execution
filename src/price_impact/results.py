...
Given a :class:`backtest.BacktestResult`, the helpers here compute:
  ...
  * daily / cumulative P&L on the unperturbed path (= observed mid, the
    baseline) and the perturbed (counterfactual) path where our marginal
    impact has been stripped, plus drawdown;
  * a transaction-cost-analysis (TCA) breakdown that decomposes P&L into
    well-defined components and *does not* invent heuristic numbers:
        ...
        Net P&L (unpert)      = mark-to-market on P_unpert (= observed mid)
        Gross P&L (pert)      = same position path marked on P_pert
...
def performance_metrics(
    ...
    cum = cumulative_pnl(daily, "pnl_unpert")
    dd = drawdown(cum)
    ...
    return {
        ...
        "mean_daily_pnl_unpert": float(daily["pnl_unpert"].mean()),
        "std_daily_pnl_unpert": float(daily["pnl_unpert"].std()),
        ...
        "win_rate": float((daily["pnl_unpert"] > 0).mean()),
...
def plot_cumulative_pnl(
    ...
    *,
    ...
    ax.plot(cum_unpert.index, cum_unpert.values, label=f"net (unpert) [{label}]", lw=1.5)
    ax.plot(
        cum_pert.index,
        cum_pert.values,
        label=f"gross (pert) [{label}]",
        lw=1.2,
        ls="--",
    )
    ax.axhline(0, color="k", lw=0.5)
    ax.set_ylabel("Cumulative P&L ($)")
    ax.set_title("Cumulative P&L (unperturbed vs perturbed)")
    ax.legend()
    plt.tight_layout()
...
def plot_sample_price_paths(
    ...
    *,
    ...
    axes[0].plot(d.p_unpert, label=r"$P_{unpert}$ (observed mid)", lw=1.0)
    axes[0].plot(d.p_pert, label=r"$P_{pert}$ (without our trades)", lw=1.0, ls=":")
